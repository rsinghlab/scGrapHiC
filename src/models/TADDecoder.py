...
class TADDecoder(pl.LightningModule):
    def __init__(
        self, PARAMETERS
    ):
        super(TADDecoder, self).__init__()
        ...
        self.fc2 = torch.nn.Linear(PARAMETERS['encoder_hidden_embedding_size']//2, 3, bias=True)
        
        
    def forward(self, x):
        for decoder_block in self.decoder_blocks:
            x = self.relu(decoder_block(x))
        return torch.nn.functional.sigmoid(self.fc2(F.relu(self.fc1(x))))
